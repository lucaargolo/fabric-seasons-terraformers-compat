...
base.archivesName.set("${name.split("-").let{it.subList(0, it.size-1)}.joinToString("-")}-compat")

val environment: Map<String, String> = System.getenv()
val releaseName = "${name.split("-").let{it.subList(0, it.size-2)}.joinToString(" ") { it.capitalize() }} Compat: ${name.split("-").let { it.subList(0, it.size-1) }.last().capitalize()} ${project["version"]}"
    ...
    project(closureOf<CurseProject> {
        ...
        mainArtifact(file(releaseFile), closureOf<CurseArtifact> {
            ...
            relations(closureOf<CurseRelation> {
                ...
                optionalDependency("traverse")
                optionalDependency("terrestria")
            })
        })
...
modrinth {
    ...
    dependencies {
        ...
        optional.project("traverse")
        optional.project("terrestria")
    }
}
